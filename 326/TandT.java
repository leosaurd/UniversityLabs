...
public class TandT {
	...
	private boolean possibleMove(int row, int col, int move, Character[][] game) {
		...
		if (move == left && col != leftBound && game[row][col - 1] != 'C' && game[row][col - 1] != 'x')
			return true;

		// right
		if (move == right && col != rightBound && game[row][col + 1] != 'C' && game[row][col + 1] != 'x')
			return true;

		// up
		if (move == up && row != topBound && game[row - 1][col] != 'C' && game[row - 1][col] != 'x')
			return true;

		// down
		if (move == down && row != botBound && game[row + 1][col] != 'C' && game[row + 1][col] != 'x')
			return true;
	...
	private boolean isFinished(Character[][] game) {
		String[] rows = rule.get(0).split(" ");
		String[] column = rule.get(1).split(" ");
		int[] thisRow = new int[rows.length];
		int[] thisCol = new int[column.length];
		int currentRow = 0;
		int currentCol = 0;

		for (int row = 0; row < game.length; row++) {
			int tentCounts = 0;
			currentRow = Integer.parseInt(rows[row]);
			// System.out.println("total tents for this row at " + row + " is " +
			// currentRow);
			for (int col = 0; col < game[row].length; col++) {
				int colCount = 0;
				try {
					currentCol = Integer.parseInt(column[col]);
					// System.out.println("total tents for this col at " + col + " is " +
					// currentCol);
				} catch (Exception e) {
					System.out.println("has to be numerical");
				}
				if (game[row][col] == 'C') {
					...
					tentCounts++;
					colCount++;
					thisCol[col] = thisCol[col] + colCount;

				}
				if (row == game.length - 1) {
					// System.out.println("total tents in this column " + thisCol[col]);
					if (currentCol != thisCol[col])
						return false;

				}
			}
			thisRow[row] = tentCounts;
			// System.out.println("total tents in this row " + thisRow[row]);
			// System.out.println("number of tents for this row " + currentRow);
			if (currentRow != thisRow[row])
				return false;
		}
